...
class HistorialForm(forms.Form):
		...
		start_date = forms.DateField(widget=DateInput(attrs={'class':'form-control','type': 'date','max':str(fecha_hoy),'min':'2017-01-01'}),label="Fecha Inicial")
		start_time = forms.ChoiceField(choices=HORAS_TOMA_DATOS,label='Hora Inicial')

		end_date = forms.DateField(widget=DateInput(attrs={'class':'form-control','type': 'date','max':str(fecha_hoy),'min':'2017-01-01'}), label="Fecha Final")
